...
class PlayerActivity : ComponentActivity() {
    ...
    /** 每 500ms 刷新控制条 (按钮文字/进度/时间), GONE 时也刷新(开销可忽略)。 */
    ...
    private fun updateBar() {
        val p = vod.player ?: return
        binding.btnToggle.setText(if (p.isPlaying) R.string.player_pause else R.string.player_play)
        val dur = p.duration
        if (dur > 0) {
            binding.tvTime.text = fmtTime(p.currentPosition) + " / " + fmtTime(dur)
