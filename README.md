...
- 🧩 **幂等合并**：按 `title` 唯一键去重，清单更新只覆盖 url / 元信息，不乱序
- 🎬 **双端 UI**：TV 遥控器焦点导航（菜单键一键刷新、上下选行直达默认按钮、播放页 OK 直接切换播放/暂停、返回二次确认防误触）；手机触屏卡片列表（点左上角头像进关于页，片名大字，播放/下载/删除按钮行右对齐）
...
2. 按遥控器「菜单 ☰」键直接刷新；或「刷新」按钮进刷新页，OK 触发拉取（过程逐行显示 log）；刷新成功后片库列表回到顶部（新片置顶）
3. 「设置 ⚙」按钮进设置页：双输入框可改 **APK 更新地址** 与 **片源地址**（默认值预填）
...
[启动 / 刷新 / 菜单键]
    ...
    │
    ├── LibraryStore.merge：title 唯一键匹配
    │   命中 → 覆盖业务字段，保留原入库时间
    │   未命中 → 记入库时间追加（新片置顶）
    │
